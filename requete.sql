...
SELECT T.libelleT, DATE_FORMAT(O.DateVente,'%m/%Y') 'Date', SUM(O.prix) 'Chiffre d affaire' FROM Objet O, TypeObjet T WHERE T.IdT=O.IdT AND O.DateVente IS NOT NULL GROUP BY T.libelleT, DATE_FORMAT(O.DateVente,'%m/%Y');
...
SELECT P.nomP,P.prenomP FROM Personnel P WHERE NOT EXISTS((SELECT F.IdF FROM Famille F)EXCEPT(SELECT C.IdF FROM Competences C WHERE P.NumSS=C.NumSS));
